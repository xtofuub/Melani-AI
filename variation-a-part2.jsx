    ...
    <div style={{ padding: isMobile ? '92px 24px' : isTablet ? '110px 32px' : '120px 48px', background: theme.paper, borderTop: `1px solid ${theme.rule}`, borderBottom: `1px solid ${theme.rule}` }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1fr 1fr', gap: isTablet ? 24 : 64, alignItems: 'end', marginTop: 20, marginBottom: isMobile ? 36 : 56 }}>
          ...
          >
            A small, sharp set of tools — <span style={{ color: theme.brand }}>not</span> a kitchen sink.
          </h2></Reveal>
...
function BridgeGraphic({ theme, mono }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, flexWrap: isMobile ? 'wrap' : 'nowrap', fontFamily: mono, fontSize: 11 }}>
      <div style={{ padding: '6px 10px', border: `1px solid ${theme.rule}`, borderRadius: 5, background: theme.paper, color: theme.inkSoft }}>radare2</div>
      {arrow}
      <div style={{ padding: '6px 10px', background: theme.brand, color: '#ffffff', borderRadius: 5, fontWeight: 700, letterSpacing: '0.06em' }}>MCP</div>
      {arrow}
      <div style={{ padding: '6px 10px', border: `1px solid ${theme.rule}`, borderRadius: 5, background: theme.paper, color: theme.inkSoft }}>Ghidra</div>
    </div>
  );
}
  ...
  return (
    <div style={{ padding: isMobile ? '92px 24px' : isTablet ? '116px 32px' : '140px 48px' }}>
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        ...
        >
          Three steps between <span style={{ color: theme.brand }}>a strange binary</span> and a
          finished report.
        </h2>
        </Reveal>
...
function VASecurity({ theme, serif, mono }) {
  ...
  return (
    <div style={{ padding: isMobile ? '92px 24px' : isTablet ? '116px 32px' : '140px 48px' }}>
      <div style={{ maxWidth: 1120, margin: '0 auto', display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1fr 1fr', gap: isTablet ? 32 : 80, alignItems: 'center' }}>
        <Reveal y={24}>
        <div>
          ...
          >
            Your samples stay <span style={{ color: theme.brand }}>yours.</span>
          </h2>
          <p style={{ fontSize: 16.5, color: theme.inkSoft, lineHeight: 1.65, textWrap: 'pretty' }}>
            Binder is designed for analysts working with sensitive samples. Keep work separated
            by project, manage who can see each case, and choose the model path that fits the job.
          </p>
        </div>
        </Reveal>
        <Reveal delay={150} y={30}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 2, background: theme.rule, borderRadius: 14, overflow: 'hidden', border: `1px solid ${theme.rule}` }}>
          {[
            ['Sensitive samples', 'Built for private analysis'],
            ['Access controls', 'Team-ready permissions'],
            ['Workspace data', 'Protected by design'],
            ['Model choices', 'Use cloud or local models'],
            ['Deployment path', 'Designed to scale with teams'],
            ['Team access', 'Ready for admin workflows'],
          ].map(([k, v]) => (
            <div key={k} style={{ background: theme.paper, padding: isMobile ? '20px 18px' : '24px 22px' }}>
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', maxWidth: 960, margin: '0 auto' }}>
          ...
          <Reveal y={30} duration={900} as="div">
            ...
            >
              Stop staring at <em style={{ color: 'rgba(255,255,255,0.55)', fontStyle: 'italic', fontFamily: brandMono || mono, fontSize: '0.72em', fontWeight: 700 }}>sub_401ab0</em>.
              <br />
          ...
          <Reveal y={20} duration={800} delay={180} as="div">
            <div style={{ display: 'flex', gap: 14, justifyContent: 'center', marginTop: 44, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <span style={{ color: 'rgba(250,250,249,0.25)', fontSize: 17 }}>or</span>
